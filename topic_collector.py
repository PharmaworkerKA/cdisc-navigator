"""CDISC Standards Navigator - トピック自動収集モジュール

NEWS_SOURCESからRSS + スクレイピングで海外の最新CDISC情報を収集し、
Gemini APIでCDISC関連のネタだけフィルタリング＆ランク付けする。
"""
import json
import logging
import re
...
class TopicCollector:
    ...
    def _collect_from_source(self, name, url):
        items = []
    ...
    def _filter_and_rank(self, items):
...
CDISCデータ標準（SDTM, ADaM, Define-XML等）や臨床データの規制提出に関連するものだけを抽出し、
...
【評価基準】
- CDISC標準に直接関連する記事: スコア80-100
- FDA/PMDA/EMAの臨床データ規制に関連する記事: スコア60-79
- 臨床試験データ管理全般に関連する記事: スコア40-59
- 関連性が低い記事: 除外
...
```json
[
  {{"index": 1, "score": 95, "category": "CDISC最新アップデート", "blog_title_suggestion": "【翻訳】ブログ記事タイトル案", "keyword": "SEOキーワード"}}
]
```
スコア40未満の記事は含めないでください。"""
        try:
            ...
            response_text = self._repair_json_text(response.text)
            ...
            for r in ranked:
                ...
                if 0 <= idx < len(items):
                    ...
                    item["category"] = r.get("category", "CDISC最新アップデート")
    ...
    def _simple_filter(self, items):
        keywords = [
            "cdisc", "sdtm", "adam", "define-xml", "pinnacle",
            "fda study data", "pmda", "clinical data", "ectd",
            "regulatory submission", "study data",
        ]
        result = []
        for item in items:
